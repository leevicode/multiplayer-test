...
static void UpdateBody(Body *body, float rot, Vector2 input, bool jumpPressed, bool crouchHold);
Player *findPlayer(Player *players, int uuid);
ENetPeer *connectToServer(ENetHost *client);
void mainLoop(ENetHost *client, ENetPeer *server, Player players[], Player *player, Camera *camera);
//------------------------------------------------------------------------------------
...
int main(void)
{
    ...
    // Main game loop
    printf("Player: %p, players: %p, player.x = %f\n", &player, players, player.body.position.x);
...
void mainLoop(ENetHost *client, ENetPeer *server, Player players[], Player *player, Camera *camera)
{
    while (!WindowShouldClose()) // Detect window close button or ESC key
    {
        ...
        UpdateBody(
            &(player->body),
            player->inputs.lookRotation.x,
            player->inputs.walkInput,
            player->inputs.isJumpPressed,
        ...
        for (size_t i = 0; i < MAX_PLAYERS; i++)
        {
            ...
            UpdateBody(
                &(targetPlayer->body),
                targetPlayer->inputs.lookRotation.x,
        ...
        headLerp = Lerp(headLerp, (player->inputs.isCrouching ? CROUCH_HEIGHT : STAND_HEIGHT), 20.0f * delta);
        camera->position = (Vector3){
            player->body.position.x,
            player->body.position.y + (BOTTOM_HEIGHT + headLerp),
            player->body.position.z,
...
void UpdateBody(Body *body, float rot, Vector2 input, bool jumpPressed, bool crouchHold)
{
    ...
    // More info here: https://youtu.be/v3zT3Z5az§paM?t=165
...
void GroundMove(Body *body, float rot, Vector2 input, bool jumpPressed, bool crouchHold)
{
}
...
static void DrawLevel(Player players[], Player *player)
{
    ...
    for (size_t i = 0; i < MAX_PLAYERS; i++)
    {
        ...
        // printf("drawing cube!\n");
        DrawCube(
            Vector3Add(players[i].body.position, pos),
            2.0f, 4 * height, 2.0f,
            (Color){255, 255, 0, 255});
        /* code */
    }
...
{
    PacketEvent event = (PacketEvent){
        E_UPDATEINPUTS,
        myUUID,
        {.P_updateInputs = inputs}};
